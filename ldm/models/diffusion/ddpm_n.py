...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        if k == "inpaint":
            ...
            class_tokens = batch['class_tokens']
        ...
        class_tokens = class_tokens.to(memory_format=torch.contiguous_format)
        class_attention_mask = class_attention_mask.to(memory_format=torch.contiguous_format)
        return x,inpaint,mask,reference,class_tokens,class_attention_mask
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.proj_out=nn.Linear(1280, 768)
        self.proj_out_text=nn.Linear(1024,768)
        ...
        self.instantiate_cond_stage(cond_stage_config, text_cond_stage_config)
    ...
    def instantiate_cond_stage(self, config, text_config):
        if not self.cond_stage_trainable:
            ...
            model = instantiate_from_config(config)
            text_model = instantiate_from_config(text_config)
            self.cond_stage_model = model
            self.text_cond_stage_model = text_model
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            image_c, tokens_c, attention_mask_c = c
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                image_c = self.cond_stage_model.encode(image_c)
                if isinstance(c, DiagonalGaussianDistribution):
                    image_c = image_c.mode()
            else:
                image_c = self.cond_stage_model(image_c)

            if hasattr(self.text_cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                text_c = self.text_cond_stage_model.encode(tokens_c, attention_mask_c)
                if isinstance(c, DiagonalGaussianDistribution):
                    text_c = text_c.mode()
            else:
                text_c = self.text_cond_stage_model(tokens_c, attention_mask_c)
            return image_c, text_c
        else:
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x,inpaint,mask,reference,class_tokens,class_attention_mask = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
            mask = mask[:bs]
            reference = reference[:bs]
            class_tokens = class_tokens[:bs]
            class_attention_mask = class_attention_mask[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        ...
        if self.model.conditioning_key is not None:
            ...
            c = (xc, class_tokens, class_attention_mask)
    ...
    def p_losses(self, x_start, cond, t, noise=None, ):
        ...
        # print(self.logvar, self.logvar.device, self.device, t.device)
        logvar_t = logvar[t].to(self.device)
